...
/// @title L1PoolManager - L1 端的资金池管理合约
/// @notice 管理 L1 上的用户存款、质押奖励和跨链资产转移
/// @dev 继承自 TokenBridgeBase,支持多链资产桥接和质押池管理
contract L1PoolManager is IL1PoolManager, PausableUpgradeable, TokenBridgeBase {
    using SafeERC20 for IERC20;

    /// @notice 每个质押周期的时长(默认 21 天)
    uint32 public periodTime;

    /// @notice 每个代币的质押池数组,记录所有历史池
...
//    }
    ...
    /// @notice 存款并质押 ERC20 代币
    /// @dev 将代币转入合约并记录到当前活跃的质押池中
    ...
    /// @notice 存款并质押 WETH
    /// @dev 将 WETH 从用户账户转入合约并存入当前活跃的质押池
    /// @param amount WETH 数量
    ...
    /// @notice 完成当前质押池并创建新池
    /// @dev 由 Relayer 调用,标记当前池已完成,将手续费分配给该池,并创建下一个周期的池
    /// @param CompletePools 要完成的池信息数组
    ...
    /// @notice 将 ETH 转发给 StakingManager 进行质押
    /// @dev 由 Relayer 调用,将资金从资金池转给 L1 的 StakingManager 合约进行信标链质押
    /// @param amount 质押的 ETH 数量,必须是 32 ETH 的倍数
    /// @param stakingManager StakingManager 合约地址
    /// @param batcher 批量铸造 dETH 的参数
