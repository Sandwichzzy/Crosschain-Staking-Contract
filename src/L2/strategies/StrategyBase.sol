...
contract StrategyBase is Initializable, IStrategy {
    ...
    event TransferETHToL2DappLinkBridge(
    ...
    *******************************************************************************/
    ...
    function initialize(IERC20 _stakingWeth, address _relayer, IStrategyManager _strategyManager, IL2Pauser _pauser)
        public
        virtual
        initializer
    {
    ...
    function _initializeStrategyBase(IERC20 _stakingWeth, IL2Pauser _pauser) internal onlyInitializing {
    ...
    *******************************************************************************/
    ...
    function _beforeDeposit(IERC20 weth, uint256 amount) internal virtual {
        require(
            weth == stakingWeth || address(weth) == ETHAddress.EthAddress,
            "StrategyBase.deposit: Can only deposit stakingWeth and eth"
        );
    ...
    function _beforeWithdrawal(IERC20 weth) internal virtual {
        require(
            weth == stakingWeth || address(weth) == ETHAddress.EthAddress,
            "StrategyBase.withdraw: Can only withdraw the strategy weth and eth"
        );
    }
        ...
        if (address(this).balance > 32e18) {
            ...
            bool success = SafeCall.callWithMinGas(
                ...
                abi.encodeWithSignature(
                    "BridgeInitiateETH(uint256,uint256,address)", sourceChainId, destChainId, l1StakingManagerAddr
                )
            );
    ...
    function transferWETHToL2DappLinkBridge(
        ...
        if (stakingWeth.balanceOf(address(this)) >= 32e18) {
            uint256 amountBridge =
                (stakingWeth.balanceOf(address(this)) / 32e18) * 32e18;
            nextNonce++;
            bool success = SafeCall.callWithMinGas(
                bridge,
                gasLimit,
                amountBridge,
                abi.encodeWithSignature(
                    "BridgeInitiateERC20(uint256,uint256,address,address,uint256)",
                    sourceChainId,
                    destChainId,
                    l1StakingManagerAddr,
                    wethAddress,
